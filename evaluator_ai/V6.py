...
def generate_evaluation_prompt(parameter_table, proposal_texts, rfp_text, human_evaluation_text):
    ...
    prompt.append(
        ...
        "Your final output should be a standalone, authoritative evaluation based on your expert analysis."
    )
    prompt.append(
    ...
    prompt.append(
        "Now finally ensure the entire final output is in clear, professional, and precise English."

        "All content in the final markdown table MUST be in English. This includes the 'Main Criterion' and 'Sub-Criterion' columns, which must be accurately translated from their original Arabic source."

        "For example, if an input criterion is 'خبرة فريق العمل', the output in the table must be 'Team Experience'."

        "Do not include the original Arabic text, not even in parentheses. The final report must be suitable for an English-only speaking audience. This is a critical instruction."
    )
